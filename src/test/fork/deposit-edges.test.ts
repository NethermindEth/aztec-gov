/*
 * Edge cases for deposit-via-Staker (extends deposit.test.ts).
 *
...
async function main() {
  ...
  try {
    ...
    for (let s = 0n; s < 200n; s++) {
      ...
      const v = await rpc<string>("eth_getStorageAt", [AZT, slotKey, "latest"]);
      if (v !== "0x" + "0".repeat(64)) {
        balanceSlot = s;
        break;
      }
    ...
    if (currentAllowance < depositAmount) {
      ...
      const tx = await rpc<Hash>("eth_sendTransaction", [{ from: DEV, to: AZT, data: approveData, gas: toHex(500_000n) }]);
    ...
    // deposit reverts on mainnet, but the hook's path is identical.
    ...
    const depositResult: Hash | { error: string } = await rpc<Hash>("eth_sendTransaction", [
      { from: DEV, to: GOV, data: depositData, gas: toHex(500_000n) },
    ]).catch((e: unknown) => ({ error: e instanceof Error ? e.message : String(e) }));

    if (typeof depositResult === "object" && "error" in depositResult) {
      ...
      const rcpt = await c.waitForTransactionReceipt({ hash: depositResult });
  ...
  const isDisabled = (power: bigint): boolean => power === 0n;
