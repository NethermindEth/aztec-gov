...
const AZT = getAddress("0xa27Ec0006E59F245217ff08CD52A7E8b169e62d2"); // mainnet AZT (from .env.local NEXT_PUBLIC_STAKING_ASSET_ADDRESS)
const INDEXER = "https://dgk9duhuxabbq.cloudfront.net";

// Beneficiary → ATP map, equivalent to what the indexer returns for each.
// Pre-discovered via the on-chain log scan (find-real-stuck-users.mjs).
// Used in lieu of the indexer because CloudFront WAF blocks our sandbox.
// In production, the dashboard hits the indexer and gets the same data.
const KNOWN_ATPS = {
...
// In production, this hits the indexer; here we use a known map because our
// sandbox is blocked by CloudFront WAF. The user's browser reaches the
// indexer fine; the data shape is identical to what's hardcoded here.
async function fetchBeneficiaryHoldings(beneficiary) {
  const key = beneficiary.toLowerCase();
  const atps = KNOWN_ATPS[key] ?? [];
  if (atps.length === 0) {
    // Try the real indexer in case we're running from a non-blocked network
    try {
...
// Mirror src/hooks/useUserStakers.ts: stakers come from holdings.
// Returns the list of ATPs (recipient set for log scan).
function deriveRecipients(walletAddr, holdings) {
...
async function simulateUseWithdrawals(recipients) {
  ...
  for (let to = tip; to >= endBlock; to -= CHUNK) {
    ...
    for (const logs of perRecipient) {
      for (const log of logs) if (log.args.withdrawalId != null) ids.add(log.args.withdrawalId);
    }
  }
...
function structSlot(id, baseSlot) {
  return keccak256(encodeAbiParameters([{ type: "uint256" }, { type: "uint256" }], [BigInt(id), BigInt(baseSlot)]));
}
...
const candidates = [
  "0x78FA029F04251cc810DFF72CCC7B4764DBC16899", // our canonical user (no pending originally)
  "0x256be0de90e34244bdef783de58cac27ae9ffeb3", // 4289 AZT ready
  "0x454a3a899dee11a00e05a758b486c45f3b0d829f", // 1290 AZT ready
  "0x2b9338f90182dab6d485dc2ff2e185407f17b442", // 29763 AZT ready
  "0xb6b598d182b266d071c0e80ff57abb90fdd0fb0f", // 269 AZT ready
];
...
// Test 2 + 3: Initiate as the real Staker owner via auto-impersonate and
// verify calldata identity with dashboard's useWithdraw.
...
const amount = 1234n * 10n ** 15n; // 1.234 AZT. a distinctive amount

// What the dashboard's useWithdraw would build:
...
// Now actually send it as the real user
...
const txHash = await rpc("eth_sendTransaction", [{ from: USER, to: STAKER, data: dashCalldata, gas: toHex(500_000n) }]);
...
// Verify dashboard would surface this new withdrawal
...
const slotPlus1 = "0x" + (BigInt(slotBase) + 1n).toString(16).padStart(64, "0");
...
// Now finalize from ANY OTHER account. proves permissionless
const RANDO = getAddress("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"); // anvil dev #0. totally unrelated to USER
...
const finTx = await rpc("eth_sendTransaction", [{ from: RANDO, to: GOV, data: finalizeCalldata, gas: toHex(500_000n) }]);
...
// Confirm dashboard would correctly remove it
